...
namespace AE
{
	namespace Graphics
	{
		namespace Device
		{
			ContextGL15::ContextGL15(AE::Graphics::Device::ContextDesc &contextDesc, AE::Graphics::Device::Driver *deviceDriver) 
				: Context(deviceDriver)
			{
				...
				// TODO: I really don't like this solution... Is there a way of setting the OpenGL flag AFTER SDL_Window is created?
				SDL_Window *sdlWindow;

				sdlWindow = SDL_CreateWindow(window->getTitle().c_str(), window->getPosition().x, window->getPosition().y,
					window->getDimensions().x, window->getDimensions().y, SDL_WINDOW_OPENGL | SDL_WINDOW_HIDDEN);

				SDL_SetWindowData(sdlWindow, "AnimaWindow", window);

				window->_setSdlWindow(sdlWindow);

				contextDesc.parentWindow->attachDeviceContext(this);

				mGLContext = SDL_GL_CreateContext(sdlWindow);
				assert(mGLContext != NULL);
				...
				mFrameBuffer = new AE::Graphics::Device::FrameBufferGL15(pixelBufferDesc);

				// Pipeline creation
				mPipeline = new AE::Graphics::Device::PipelineGL15(this);
			...
			bool ContextGL15::beginRendering()
			{
			...
			/*AE::Graphics::Surface* ContextGL15::createOffscreenSurface(const AE::Math::Point2 &dimensions, AE::Graphics::SurfaceFormat surfaceFormat, bool usesVideoMemory)
			{
				if(usesVideoMemory)
				{
